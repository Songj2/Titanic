...
def titanic_rate(request):
    dataset= Passenger.objects.values('ticket_class')\
        .annotate(survived_cnt=Count('ticket_class', filter=Q(survived=True)),
                  not_survived_cnt=Count('ticket_class', filter=Q(survived=False)),
                  survived_rate=100 * Count('ticket_class', filter=Q(survived=True)) / Count('ticket_class'))

    # dataset_rate= Passenger.objects.values('ticket_class')\
    #     .annotate(survived_rate=100 * Count('ticket_class', filter=Q(survived=True)) / Count('ticket_class'))

    categories = list()
    survived_series_data = list()
    not_survived_series_data = list()
    survived_rate_series_data= list()

    for entry in dataset:
        categories.append('%s Class' % entry['ticket_class'])  # for xAxis
        survived_series_data.append(entry['survived_cnt'])  # for series named 'Survived'
        not_survived_series_data.append(entry['not_survived_cnt'])  # for series named 'Not survived'
        survived_rate_series_data.append(entry['survived_rate'])  #for series named 'Survived rate
    ...
    survived_rate_series = {
        'type':'line',
        'name': 'Survived rate',
        'data': survived_rate_series_data,
        'color': 'blue'
    }

    chart = {
        # 'chart': {'type': 'column'},
        'title': {'text': 'Titanic Survivors and survived rate by Ticket Class'},
        'xAxis': {'categories': categories},
        'series': [survived_series, not_survived_series, survived_rate_series]
    }

    dump = json.dumps(chart)

    return render(request, 'titanic_rate.html', {'chart': dump})
